...
#include <CL/cl.h>
#include <omp.h>
// Понедельник 06.12.2021 в 20:00 
//https://teams.microsoft.com/l/meetup-join/19%3ameeting_N2E1ZWJmOGMtOTMyOC00ZDAzLWJlZDQtMDA5ZWI1MzkzYzdh%40thread.v2/0?context=%7b%22Tid%22%3a%2246c98d88-e344-4ed4-8496-4ed7712e255d%22%2c%22Oid%22%3a%22eaa4430a-8d8a-4c5b-b55c-6a9ac9fba3f7%22%7d
using namespace std;

template <class T>
T* axpy_openclCPU(int length, T _a, T* data, int _incx, T* result, int _incy, cl_device_id device, size_t block_size) {
    cl_context context;
...
int main()
{
    ...
    start = omp_get_wtime();
    r1 = axpy_openclCPU<float>(length, 2, data1, 1, result1, 1, CPU, block_size);
    end = omp_get_wtime();
    cout << "saxpy result: " << end - start << " \n";

    start = omp_get_wtime();
    r2 = axpy_openclCPU<double>(length, 2, data2, 1, result2, 1, CPU, block_size);
    end = omp_get_wtime();
    cout << "daxpy result: " << end - start << " \n";
